...
# ==============================================================================
# 2. ALPHA ENGINE SIGMA v2.1 (Infinite Discovery & Persistence)
# ==============================================================================
...
class AlphaEngineSigma:
    ...
    def evaluate_strategy(self, df_test, params=None):
        ...
        # Trade Analysis for Report
        trades = []
        if len(res_df) > 0:
            trade_start_idx = -1
            for i in range(len(res_df)):
                if signals[i] > 0 and trade_start_idx == -1: # Entry
                    trade_start_idx = i
                elif signals[i] == -1 and trade_start_idx != -1: # Exit
                    # Trade PnL is product of (1 + returns) from entry day to exit day
                    t_pnl = (1 + res_df['Strategy_Return'].iloc[trade_start_idx:i+1]).prod() - 1
                    trades.append({'year': res_df['Year'].iloc[i], 'pnl': t_pnl})
                    trade_start_idx = -1

        met = {'total_return': cum_s.iloc[-1]-1, 'sharpe': (strat_returns.mean()/(strat_returns.std()+1e-9))*np.sqrt(252),
               'mdd': ((cum_s.cummax()-cum_s)/(cum_s.cummax()+1e-9)).max(), 'trades': trades}
        return res_df, met

    def generate_report(self, res_df, met):
        # --- Precision Alignment Helper ---
        def get_width(s):
            return sum(2 if ord(c) > 127 else 1 for c in str(s))

        def pad(s, width, align='center'):
            s = str(s)
            diff = width - get_width(s)
            if diff <= 0: return s
            if align == 'center':
                l_pad = diff // 2
                r_pad = diff - l_pad
        ...
        print("\n" + "="*85)
        print(f" ALPHA ENGINE SIGMA v2.1 RE-EQUILIBRATED REPORT ({self.name})")
        print("="*85)
        
        trades_df = pd.DataFrame(met['trades'])
        if not trades_df.empty:
            wins = trades_df[trades_df['pnl'] > 0]['pnl']
            losses = trades_df[trades_df['pnl'] <= 0]['pnl']
            avg_win = wins.mean() if not wins.empty else 0
            avg_loss = abs(losses.mean()) if not losses.empty else 0
            t_pl_ratio = avg_win / (avg_loss + 1e-9)
            t_pl_str = f"{t_pl_ratio:.2f}" if avg_loss > 0 else "손실없음"
        else:
            t_pl_str = "0.00"

        summary = f" [종합 실적] 누적 수익률: {met['total_return']:.2%} | 샤프: {met['sharpe']:.2f} | MDD: {met['mdd']:.2%} | 손익비: {t_pl_str}"
        print(summary)
        
        yr_rets = res_df.groupby(res_df.index.year)['Strategy_Return'].apply(lambda x: (1+x).cumprod().iloc[-1]-1)
        
        print("\n [연도별 세부 실적 일람표]")
        col_widths = [12, 16, 18, 14, 16]
        headers = ["해당연도", "수익률", "평균손익비", "적중율", "평균거래건수"]
        
        header_line = " " + " | ".join(pad(h, w) for h, w in zip(headers, col_widths))
        div = "-" * 85
        print(div); print(header_line); print(div)
        
        all_positive = True
        for y, r in yr_rets.items():
            if r < 0: all_positive = False
            y_trades = trades_df[trades_df['year'] == y] if not trades_df.empty else pd.DataFrame()
            
            if not y_trades.empty:
                cnt = len(y_trades)
                wr = f"{len(y_trades[y_trades['pnl'] > 0]) / cnt:.2%}"
                wins = y_trades[y_trades['pnl'] > 0]['pnl']
                losses = y_trades[y_trades['pnl'] <= 0]['pnl']
                avg_win = (wins.mean() if not wins.empty else 0)
                avg_loss = abs(losses.mean()) if not losses.empty else 0
                pl_str = f"{avg_win / (avg_loss + 1e-9):.2f}" if avg_loss > 0 else "손실없음"
            else:
                cnt = 0; wr = "0.00%"; pl_str = "0.00"
                
            row_data = [f"{y}년", f"{r:.2%}", pl_str, wr, str(cnt)]
            row_line = " " + " | ".join(pad(d, w) for d, w in zip(row_data, col_widths))
            print(row_line)
            
        print(div)
        if all_positive: print(" [*] STABILITY ACHIEVED: 전 연도 수익 달성 (No Loss Years).")
        else: print(" [!] Warning: 손실 발생 연도가 포함되어 있습니다.")
        print("="*85)
    ...
    def run_persistence_evolution(self, num_rounds=1):
        ...
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(f"=== ALPHA ENGINE REPORT v2.1: {self.name} ({self.ticker}) ===\n Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
