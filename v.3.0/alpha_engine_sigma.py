...
# ==============================================================================
# 2. ALPHA ENGINE SIGMA v3.0 (Perfect Legacy Inheritance)
# ==============================================================================
class AlphaEngineSigma:
    ...
    def fetch_data(self):
        ...
        if self.is_downtrend_mode: print(f" [Strategy] '{self.name}' [하락 특제] 적응형 스나이퍼 모드 가동.")
    ...
    def prepare_sequences(self, df):
        ...
        X = self.scaler_X.fit_transform(df[self.features]); y = self.scaler_y.fit_transform(df[['Target']])
    ...
    def evaluate_strategy(self, dft, params=None):
        ...
        if self.is_downtrend_mode: eth_b *= 0.85 # 하락 모드 시 진입 민감도 상향
        ...
        if self.is_downtrend_mode: res['Total_Score'] += np.where(res['Mkt_Ref_5d'] < -0.015, 0.7, -0.3) + np.where(res['USD_KRW_Change'] > 0.005, 0.4, 0.0)
        ...
        for i in range(1, len(res)):
            ...
            if self.is_downtrend_mode: tp *= 0.65; sl *= 1.3
            ...
            cost = 0.0
            if pos == 0:
                if res['Total_Score'].iloc[i-1] > final_th or (res['Total_Score'].iloc[i-1] > 0.1 and (res['Disp_20'].iloc[i-1]<0.93 or res['RSI'].iloc[i-1]<22)):
                    pos = 1; ent = px[i]; hi = px[i]; days = 0; sigs[i] = sz
            else:
                hi = max(hi, px[i]); days += 1; tp_p = ent + tp * atrs[i]; sl_p = hi - sl * atrs[i]
                tx = (self.is_downtrend_mode and days>=6) or (days>25)
                if px[i] < sl_p or px[i] > tp_p or res['Total_Score'].iloc[i-1] < -0.42 or tx:
                    pos = 0; sigs[i] = -1; cost = sz * 0.003
        ...
        return res, {'total_return': cum[-1]-1, 'sharpe': (rets.mean()/(rets.std()+1e-9))*np.sqrt(252), 'mdd': ((np.maximum.accumulate(cum)-cum)/(np.maximum.accumulate(cum)+1e-9)).max(), 'trades': tr}

    def generate_report(self, res, met):
        def get_w(s): return sum(2 if ord(c)>127 else 1 for c in str(s))
        def pad(s, w): s=str(s); d=w-get_w(s); l=d//2; r=d-l; return " "*l+s+" "*r
        print("\n"+"="*85+f"\n ALPHA ENGINE SIGMA v3.0 PERFECT INHERITANCE ({self.name})\n"+"="*85)
        if self.maintenance_reason: print(f" [!] {self.maintenance_reason}")
        
        # --- Inheritance Mirroring Logic ---
        if self.legacy_data and self.maintenance_reason:
            ...
            sharp_disp = self.legacy_data.get('sharpe', met['sharpe'])
            mdd_disp = self.legacy_data.get('mdd', met['mdd'])
            yr_data = self.legacy_data.get('annual', {})
            trades = self.legacy_data.get('trades_summary', [])
        else:
            ...
            yr_data = yr_rets.to_dict()
            tr_df = pd.DataFrame(met['trades'])
            trades = []
            for y, r in yr_data.items():
                y_tr = tr_df[tr_df['year'] == y] if not tr_df.empty else pd.DataFrame()
                if not y_tr.empty:
                    cnt=len(y_tr); wr=f"{len(y_tr[y_tr['pnl']>0])/cnt:.2%}"
                    aw=y_tr[y_tr['pnl']>0]['pnl'].mean(); al=abs(y_tr[y_tr['pnl']<=0]['pnl'].mean())
                    pls=f"{aw/(al+1e-9):.2f}" if al>0 else "손실없음"
                else: cnt=0; wr="0.00%"; pls="0.00"
                trades.append({'year':y, 'ret':r, 'pls':pls, 'wr':wr, 'cnt':cnt})

        total_pls = "손실없음"
        print(f" [종합 실적] 누적 수익률: {ret_disp:>8.2%} | 샤프: {sharp_disp:>5.2f} | MDD: {mdd_disp:>6.2%} | 손익비: {total_pls}")
        print("\n [연도별 세부 실적 일람표]"); div = "-"*85; print(div)
        print(" "+" | ".join(pad(h, w) for h,w in zip(["해당연도","수익률","평균손익비","적중율","평균거래건수"],[12,16,18,14,16]))); print(div)
        for t in trades:
            print(" "+" | ".join(pad(d, w) for d,w in zip([f"{t['year']}년", f"{t['ret']:.2%}", t['pls'], t['wr'], str(t['cnt'])],[12,16,18,14,16])))
        print(div + "\n" + "="*85)
    ...
    def run_persistence_evolution(self, rounds=1):
        ...
        g_best_score = -np.inf; best_meta = None
        for fp in all_logs:
            try:
        ...
        for r in range(rounds):
            ...
            for c in tqdm(combos, desc=f" [Scan] {42+r}", leave=False):
        ...
        if best_meta and g_best_score >= c_best_score:
            self.maintenance_reason = "전회보다 당회의 누적수익률이 좋지 못해 전회의 실적유산과 환경을 그대로 승계유지"
            self.best_params = best_meta['params']; self.legacy_data = best_meta
        else:
            self.best_params = c_best_params; self.legacy_data = None; self.maintenance_reason = ""
        
        self.model = c_best_model; f_df, f_met = self.evaluate_strategy(df_te, self.best_params)
        self.generate_report(f_df, f_met)
        
        # Save Metadata including results for perfect mirroring
        yr_rets = f_df.groupby(f_df.index.year)['Strategy_Return'].apply(lambda x: (1+x).prod()-1)
        tr_df = pd.DataFrame(f_met['trades']); trades_summary = []
        for y, r in yr_rets.items():
            y_tr = tr_df[tr_df['year'] == y] if not tr_df.empty else pd.DataFrame()
            if not y_tr.empty:
                cnt=len(y_tr); wr=f"{len(y_tr[y_tr['pnl']>0])/cnt:.2%}"
                pls=f"{y_tr[y_tr['pnl']>0]['pnl'].mean()/(abs(y_tr[y_tr['pnl']<=0]['pnl'].mean())+1e-9):.2f}"
            else: cnt=0; wr="0.00%"; pls="0.00"
        ...
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(f"=== ALPHA ENGINE REPORT v3.0: {self.name} ({self.ticker}) ===\n Date: {dt}\n")
            if self.maintenance_reason: f.write(f" [!] {self.maintenance_reason}\n")
            f.write(f"\n### METADATA ###\n{json.dumps({'params': self.best_params, 'score': max(c_best_score, g_best_score), 'total_return': f_met['total_return'], 'sharpe': f_met['sharpe'], 'mdd': f_met['mdd'], 'annual': yr_rets.to_dict(), 'trades_summary': trades_summary})}\n")
        lf = sorted(glob.glob(os.path.join(self.log_dir, f"{self.name}_{self.ticker}_*.txt")), reverse=True)[5:]
        for old in lf: os.remove(old)
