...
# ==============================================================================
# 2. ALPHA ENGINE SIGMA v3.1 (Perfect Legacy Inheritance)
# ==============================================================================
class AlphaEngineSigma:
    ...
    def evaluate_strategy(self, dft, params=None):
        ...
        for i in range(1, len(res)):
            ...
            if pos == 0:
                ...
                if px[i] < sl_p or px[i] > tp_p or res['Total_Score'].iloc[i-1] < -0.4 or tx:
    ...
    def generate_report(self, res, met):
        ...
        print("\n"+"="*85+f"\n ALPHA ENGINE SIGMA v3.1 PERFECT INHERITANCE ({self.name})\n"+"="*85)
        ...
        if self.legacy_data and self.maintenance_reason:
            ...
            # Fallback for old logs missing structured annual data
            if not yr_data or not trades:
    ...
    def run_persistence_evolution(self, rounds=1):
        ...
        # 2. NO-REGRESSION EVALUATION (Compare against GLOBAL best - ONLY positive legacy)
        if best_meta and g_best_score > 0 and g_best_score >= c_best_score:
            ...
            if best_meta and g_best_score <= 0:
                print(f" [!] 과거 기록이 손실 상태이므로 승계를 거부하고 금회 성과를 채택합니다.")
        ...
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(f"=== ALPHA ENGINE REPORT v3.1: {self.name} ({self.ticker}) ===\n Date: {dt}\n")
