...
## [V1.0 Master] (2026-02-09) - Alpha Ultimate Equilibrator
**상태**: ACTIVE (New Baseline Era)
**내용**: 사용자 제공 v7 Ultimate 엔진을 프로젝트의 새로운 표준인 V1.0으로 지정함.
- **AI 엔진**: Transformer (Self-Attention) 기반 심층 시계열 예측
- **통합 데이터**: 종목 시세 + VIX(공포지수) + USD/KRW(환율) 다차원 분석
- **자금 관리**: 켈리 공식(Kelly Criterion) 기반 동적 포지션 사이징
- **리스크 관리**: 매도 시 0.3% 수수료 반영 및 전개년 안정성 중심 최적화
- **보관 위치**: `old_versions/V1_Ultimate/`

---

## v6.0 [RESTORATION] (2026-02-08) - Dynamic Master Final
**상태**: 완전 복원 (LSTM Model + Floor-First Optimization)
**내용**: v7.x의 Transformer 및 불순물 로직을 제거하고, 황금기 성능(누적 수익률 수천 %)을 냈던 v6.0 Dynamic Master 코드로 프로젝트 전체를 롤백함.
- LSTM 모델 기반 AI 지휘자 복구
- 전개년 10% 하한선 방어(Floor-First) 전략 복구
- RSS (Regime-Specific Scalping) 트레이딩 로직 복구
- 종목 선택 메뉴 유지

---

## v7.7 (2026-02-08) - Streamlit Cloud Ready [DISCARDED]
**목표**: 적중률 60% 이상 달성
